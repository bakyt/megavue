    ...
    <div class="box box-default">
        <div class="box-body">
            <form class="form-horizontal" onsubmit="event.preventDefault()">
                ...
                <div class="form-group">
                    <label for="role" class="col-sm-2 control-label">Роль для админки</label>

                    <div class="col-sm-10">
                        <select name="role_name" id="role" class="form-control">
                            <option value="0">Нет доступа</option>
                            <option v-for="role in roles" v-bind:key="role.id" :value="role.name" :selected="user.role?(role.name===user.role.name):false" >{{ role.display_name }}</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                        ...
                        <button v-if="permissions.indexOf('tool-users.destroy') !== -1" type="button" @click="removeUser" class="btn btn-danger">Удалить</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    export default {
        ...
        methods:{
            fetchUser(){
                axios.get('/api/tool-users/'+this.userId+'/edit')
                    .then(res=>{
            ...
            updateUser(){
                ...
                axios.put('/api/tool-users/'+this.userId, object)
                    .then(()=>{
                        new PNotify({
                            title: 'Успех',
                        ...
                        this.$router.push({ name:'tool-users.index' });
                    })
            ...
            removeUser(){
                ...
                (new PNotify({
                    ...
                    axios.delete('/api/tool-users/'+userId);
                    new PNotify({
                        title: 'Успех',
                    ...
                    $router.push({ name:'tool-users.index' });
                })
            }
        }
    }
