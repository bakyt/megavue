                        ...
                        <div class="row row-sm-offset">
                            ...
                            <div class="col-md-12 multi-horizontal">
                                ...
                                <div class="form-group">
                                    ...
                                    <router-link :to="{ name:'stores.view', params:{ slug:slug } }" class="btn btn-default">{{ $lang.app.cancel }}</router-link>
                                    <button class="btn btn-danger" type="button" @click="deleteStore"><i class="fa fa-trash-alt"></i> {{ $lang.app.delete }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            fetchStore () {
                return axios.get('/api/stores/'+this.slug+'/edit')
                    .then(response => {
                        this.storeTypes = response.data.storeTypes;
                        return response.data;
                    })
                    .then(response=>{
                        this.types = response.data.types.map(type=>type.id);
                        this.name = response.data.name;
                        this.description = response.data.description;
                        this.about = response.data.about;
                        this.address = response.data.address;
                        if(response.data.address_map) {
                            this.markerLatLng = response.data.address_map;
                            const latLngs = [this.markerLatLng ];
                            this.bounds = L.latLngBounds(latLngs);
                        }
                        let contacts = response.data.contacts;
                        contacts.phones = contacts.phones.map(phone=>{
                            return {value: parseInt(phone.replace('+', ''))};
                        });
                        contacts.whatsapp = contacts.whatsapp.replace('+', '');
                        this.contacts = contacts;
                        this.iconImageForNow = response.data.icon;
                        this.backgroundImageForNow = response.data.background_image;
                    })
            },
            loadBreadcrumbs(){
                this.breadCrumbing([
                    ...
                    {
                        text: this.name + ':' + this.$lang.store.settings,
                        icon: 'fa fa-cog',
                        hideTitle: true
                    }
                ])
            ...
            saveStore(e){
                ...
                if (this.types.length<1){
                    ...
                    if((contacts.whatsapp+'')[0] !== '0') contacts.whatsapp = '+'+this.contacts.whatsapp;
                    ...
                    axios.post('/api/stores/'+this.slug, formData, {
                        headers: {
                        ...
                        .then(response => {
                            new PNotify({
                                ...
                                text: this.$lang.app.successfully_updated,
                                icon: 'fa fa-check',
                                type: "success"
                            });
                            this.$auth.init();
            ...
            deleteStore(){
                if(confirm(this.$lang.app.delete_confirm)) axios.delete('/api/stores/'+this.slug)
                    .then(()=>{
                        new PNotify({
                            title: this.$lang.app.success,
                            text: this.$lang.app.successfully_deleted,
                            icon: 'fa fa-check',
                            type: "success"
                        });
                        this.$store.dispatch('removeStore', this.slug);
                        this.$router.push({ name:'home' })
                    })
            }
        }
